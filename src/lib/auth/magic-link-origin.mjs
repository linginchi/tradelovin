...
import { getMagicLinkAllowedHosts, MAINLAND_FALLBACK_ORIGIN } from "../site-entries.mjs";
...
export function isAllowedMagicLinkHost(hostname) {
	const host = String(hostname ?? "")
		...
		.toLowerCase()
		.split(":")[0];
	return getMagicLinkAllowedHosts().includes(host);
}
...
export function resolveMagicLinkBaseUrl({
	...
	return normalizeOrigin(fallbackOrigin) || MAINLAND_FALLBACK_ORIGIN;
}
