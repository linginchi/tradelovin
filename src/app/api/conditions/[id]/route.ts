...
import { deleteCondition } from "@/lib/trade-v2/watch-conditions";
...
export async function DELETE(_: Request, { params }: Params) {
	...
	try {
		await deleteCondition(ctx.supabase, ctx.userId, id);
		return NextResponse.json<ApiAckResponse>({ success: true });
	} catch (error) {
		return NextResponse.json(
			{ success: false, error: error instanceof Error ? error.message : "删除条件单失败" } satisfies ApiErrorResponse,
			{ status: 400 },
		);
	}
}
