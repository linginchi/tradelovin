...
function rejectIfObjectStoreRequired(storageKey: string): NextResponse | null {
	if (objectStoreMissingFor(storageKey)) {
...
export async function GET(_request: Request, { params }: RouteContext) {
	...
	if (error) {
		...
		if (/published_at/i.test(error.message)) {
			...
			const blocked = rejectIfObjectStoreRequired(String(fallback.data.storage_key));
	...
	const blocked = rejectIfObjectStoreRequired(String(video.storage_key));
	if (blocked) return blocked;
