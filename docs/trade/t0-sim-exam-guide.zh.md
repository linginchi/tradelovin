...
额度由 **P3 · 金钱豹教练** 在考核盘「资源」栏设置并批准，后台不逐笔审批，也不必进后台加日常库存。

```mermaid
flowchart TD
  student["学员在资源栏提出添加"] -->|"审核中"| coachSee["教练在同一栏看到待批"]
  coachInv["教练在资源栏加可发放库存"] --> pool["tq_coach_resources"]
  coachSee -->|"批准（库存不够则先补）"| grant["发放并记下批准人"]
  coachDirect["教练直接发放"] --> grant
  grant --> personal["个人资源额度"]
  personal -->|"开多买入 / 开空卖出"| fill[成交并扣额度]
```

### 学员申请额度（逐步）

1. 滚到「资源」，选择一位金钱豹教练并点「申请绑定」，等教练接受。
2. 输入标的代码（库存里没有的也可以）和「申请数量（股）」，例如 `1000`。
3. 做多点 **申请多头额度**；做空点 **申请空头额度**。
4. 此时个人表不会立刻增加，状态为 **审核中**。
5. 教练当场批准后，「个人资源额度」出现记录，才能按同一方向下单。

### 教练怎么设置

1. 管理员在后台「讲师」页勾选或按邮箱任命「金钱豹教练」（需有效 T3）。
2. 教练打开考核盘「资源」栏：加入标的与可做多/可做空数量。
3. 可批准名下审核中申请（库存不足会按申请数量先补再发），或直接给已绑定学员发放。教练给自己加额度不必绑定另一位教练，发放会记入单据。
4. `/coach` 只是备份工作台；后台「模拟盘资源」是全站应急模板，日常不要在那里加。
...
| 你看到 | 先做什么 |
|---|---|
| 空头/多头额度不足 | 等教练批准个人额度到账后再下单 |
| 请先选择金钱豹教练 | 资源面板绑定教练 |
| 教练库存中不存在该标的 | 在资源栏输入该代码提出添加，等审核通过 |
| 请先输入标的 | 顶部输入代码并回车 |
...
3. 绑定教练，输入标的并申请空头 `1000` → 审核中 → 教练在同一栏批准后个人表出现记录。
4. 再开空 → 不应再因额度被拒。
...
资源页：左边教练可发放库存，右边个人额度（真正能用来开仓的数量）。申请后状态为审核中，等教练在本栏批准。
