...
额度由 **P3 · 金钱豹教练** 设置并批准，后台不逐笔审批。

```mermaid
flowchart TD
  coach["教练工作台库存"] -->|"批准或直接发放"| personal["个人资源额度"]
  student["学员在资源面板申请"] -->|"待批准"| coach
...
2. 点左侧「教练可发放库存」那一行选标的。
3. 填写「申请数量（股）」，例如 `1000`。
4. 做多点 **申请多头额度**；做空点 **申请空头额度**。
5. 此时个人表不会立刻增加，状态为待教练批准。
6. 教练批准后，「个人资源额度」出现记录，才能按同一方向下单。

### 教练怎么设置

1. 管理员在 `/cjkzt/instructors` 勾选或按邮箱任命「金钱豹教练」（需有效 T3）。
2. 教练打开 `/coach`：加入标的与可做多/可做空数量。
3. 可以直接给已绑定学员发放，也可以批准学员申请。
...
| 请先选择金钱豹教练 | 资源面板绑定教练 |
| 教练库存中不存在该标的 | 点教练库存已有行，或请教练加入该代码 |
| 请先输入标的 | 顶部输入代码并回车 |
...
3. 绑定教练，点库存标的，申请空头 `1000` → 待批准 → 教练批准后个人表出现记录。
4. 再开空 → 不应再因额度被拒。
